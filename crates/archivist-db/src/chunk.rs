...
/// Varsayilan parca boyutu (kelime) — H2 pariti. MiniLM 512-token penceresine kendisi kirpar
/// → kelime-bazli yaklasik boyut yeterli (gercek BPE gerekmez).
...
pub fn chunk_text(text: &str, chunk_words: usize, overlap: usize) -> Vec<String> {
    ...
    // Sus/ayrac bloklari (>=2 ardisik) → bosluk. Unicode kutu-cizim/geometrik/dingbat araliklari
    // + sik gorulen tek-glifler. replacement char (U+FFFD) → bosluk.
    let cleaned: String = {
...
impl Db {
        ...
        Ok(self.conn.query_row(
            "SELECT count(*) FROM assets a
             WHERE a.deleted_at IS NULL
               AND NOT EXISTS (SELECT 1 FROM text_chunks c WHERE c.asset_id = a.id)
               AND NOT EXISTS (SELECT 1 FROM index_skips s
                               WHERE s.asset_id = a.id AND s.stage = ?1)",
            params![IndexStage::Chunk.as_str()],
            |r| r.get(0),
        )?)
    }

    /// Indekslenmis (en az bir chunk'i olan) asset sayisi.
    pub fn chunked_asset_count(&self) -> Result<i64, DbError> {
        Ok(self.conn.query_row(
            "SELECT count(DISTINCT asset_id) FROM text_chunks",
            [],
            |r| r.get(0),
        )?)
    }
    ...
    /// chunk_index ARTAN → metadata chunk (-1) once, sonra govde 0,1,2... Salt-okuma; vektor/FTS
    /// dondurulmez (yalniz metin + kaynak). Cope/aktiflik filtresi YOK (detay zaten asset'i acti).
    pub fn chunks_for_asset(&self, asset_id: i64) -> Result<Vec<AssetChunk>, DbError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, chunk_index, page, text FROM text_chunks
             WHERE asset_id = ?1 ORDER BY chunk_index, id",
        )?;
    ...
    pub fn assets_without_chunks(
        ...
        let rows = stmt.query_map(params![after_id, IndexStage::Chunk.as_str(), limit], |r| {
            Ok(PendingChunk {
                id: r.get(0)?,
                file_name: r.get(1)?,
                ext: r.get(2)?,
    ...
    pub fn set_asset_chunks(&self, asset_id: i64, chunks: &[ChunkWrite]) -> Result<(), DbError> {
        ...
        for c in chunks {
            tx.execute(
                "INSERT INTO text_chunks(asset_id, chunk_index, page, text)
                 VALUES (?1, ?2, ?3, ?4)",
                params![asset_id, c.chunk_index, c.page, c.text],
            )?;
