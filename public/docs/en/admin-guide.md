...
> Version 3.0.0 | 2026-05-23 — This guide is for Admin (full-privilege) users only.

> **v3.0.0 Quick Summary for Admins:** This release introduces the **V3 Schema Architecture** — vector data (embeddings, text chunks, relations, FTS) moved to a separate `archivist_vec.db` file. Existing archives auto-migrate on first launch (with `.bak` backup). Update your backup workflow to account for the new file layout. See "4a. V3 Schema Architecture" below.
...
- **Main Archive (shared)** — managed by the admin, read-only for viewers
  - Main DB: `archivist.db` (default: `AppDataDir/archivist.db`)
  - Vector DB: `archivist_vec.db` (v3.0.0+ — same folder)
- **Local Archive (personal)** — per-user
  - Main DB: `archivist_local.db`
  - Vector DB: `archivist_local_vec.db` (if present)

---

## 4a. V3 Schema Architecture (3.0.0+)

![V3 Migration Panel](img/settings-v3-migration.png)

Starting with v3.0.0, ArchivistPro stores archive data in **two separate
SQLite files**:

| File | Contents | Typical Size |
|---|---|---|
| `archivist.db` | Asset metadata, tags, collections, settings, users, relation types | 50-100 MB |
| `archivist_vec.db` | Embedding vectors, text chunks, asset relations, FTS5 keyword index | 100-300 MB |

### Why the Split?

- **Performance:** Main `archivist.db` is 3-4× smaller → faster sql.js
  load and writes. Real measurement: 181 MB → 52 MB.
- **Memory:** sql.js loads the DB fully into RAM. Smaller main DB =
  less RAM, fewer blocking saves.
- **Scale:** Hundreds of thousands of embeddings and thousands of text
  chunks live in a dedicated structure with HNSW ANN index; the main
  metadata DB stays fast.
- **Maintainability:** Vector data is reindexable; metadata is not —
  separating the two makes backup strategies more flexible.

### Automatic Migration

Archives from earlier versions are migrated automatically on first
launch:

1. On startup, `PRAGMA user_version` is checked.
2. If `user_version < 3`, `runV3EpochMigration` is triggered.
3. **Premigrate backup:** `archivist.db` is copied to
   `archivist_premigrate_v3.db.bak` for rollback.
4. Embeddings / text_chunks / relations are moved to `archivist_vec.db`
   (epochs 1 → 2 → 3 sequentially).
5. Verification: do source and destination row counts and contents
   match exactly?
6. On success: old tables in the main DB are removed with
   `DROP TABLE` + `VACUUM` + `PRAGMA user_version = 3`.
7. Atomic write to disk.

> Migration runs **on the Rust side** (rusqlite). 180+ MB monoliths are
> not at risk of Tauri IPC payload limits.

### Manual Trigger (Admin Panel)

If the automatic migration fails or admin control is preferred:

**Settings → Storage → V3 Schema Migration** panel (admin-only):

- Shows current epoch (0/1/2/3) and target epoch (3).
- "Start Migration" button — admin-triggered manual flow.
- Step-by-step progress: premigrate backup → migrate 1/3 → migrate 2/3 →
  migrate 3/3 → finalize → reload.
- On error: stops, shows message, data remains intact (rollback
  available via `.bak`).

### Opt-Out

The V3 behavior can be disabled via a flag (for development/testing):

```js
localStorage.setItem('ARCHIVIST_V3_EPOCH', 'off');
```

When `off`, automatic migration is not triggered. **This is only
meaningful if set BEFORE the first migration**; reverting after a
successful migration requires restoring the `.bak` file.

### Rollback

If a problem is discovered post-migration:

1. Close the application.
2. Move or delete `archivist_vec.db`.
3. Rename `archivist_premigrate_v3.db.bak` to `archivist.db` (backup
   the current `archivist.db` first).
4. Re-open the application — it returns to the old (epoch=0) state.

### WAL Journal and Network Shares (UNC)

With v3.0.0, WAL journal mode is **on by default** — write performance
on local disk improves by ~30%. Network shares are auto-detected and
fall back to DELETE mode:

- **UNC path (`\\server\share\archive\...`):** WAL unsafe (NetBIOS
  lock semantics) → DELETE mode forced.
- **Mapped network drive (`Z:\archive\...` but actually UNC):**
  detected via `GetDriveTypeW`, DELETE mode forced.
- **Local disk:** WAL stays on.
...
If you actively work on UNC, **open the share directly as UNC** (not
as a mapped drive letter) — Windows NetBIOS junctions are an edge
case. For larger multi-user setups, this mode is an alternative to
the HTTP LAN sharing.
...
> **v3.0.0 Note:** A snapshot now saves **both `archivist.db` and
> `archivist_vec.db` together**. Restore reverts both files atomically —
> V3 data (embeddings/chunks/relations) is not lost from a snapshot.
...
- **v3.0.0:** If migration or disk-write fails, the flow halts and in-memory state is realigned with disk (false-success prevented)
- **v3.0.0:** Per-archive write lock — no write conflicts when multiple archives are used concurrently
- **v3.0.0:** Stale WAL cleanup — orphaned `-wal`/`-shm` files are removed after blob-overwrite
...
- **v3.0.0:** **Full content** in 5 languages (TR source, EN/AR/JA/ZH translations). Each tab in the help panel loads in the application's active language.
- Interface translations: **5 languages 100%** (TR, EN, ZH, JA, AR — 2310+ keys)
- AR/JA/ZH versions were produced under a professional translation procedure; the shared glossary `docs/i18n-glossary.md` ensures consistency.
...
- **Release Notes** appear as a fourth tab in the help panel, sourced from `docs/CHANGELOG.md`.

---

## 16. Version Bump and Release Checklist (Admin)

When distributing a new version, follow this checklist. Full text:
`docs/RELEASE_CHECKLIST.md`.

Per release:

- Update `src/appVersion.ts` → `APP_VERSION` + `APP_BUILD_DATE`
- Sync version fields in `src-tauri/tauri.conf.json`,
  `src-tauri/Cargo.toml`, `package.json` (`cargo check` auto-updates
  `Cargo.lock`)
- Add a new section to `docs/CHANGELOG.md`
- Update the date on every-language help docs (user-guide, admin-guide,
  scenarios)
- `npx tsc --noEmit` + `npm test` + `cargo test --features admin --lib`
  must be green
- Run MSI build (`npm run offline:build`) — produce artifact, attach
  to GitHub Release
