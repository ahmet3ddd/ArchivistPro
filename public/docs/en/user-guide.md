...
ArchivistPro is a desktop application for intelligently archiving, searching, and managing your architectural files. **It runs fully offline** — your files never leave your computer.

![Main Window](img/main-window.png)

> **Upgrading to v3.0.0:** If you are coming from an earlier version, on first launch your archive is **automatically migrated to the V3 schema**. This takes a few seconds and requires no action from you. A backup file named `archivist_premigrate_v3.db.bak` is kept alongside the archive for rollback. See the "Data Safety" section below.
...
![Source Folders](img/sidebar-source-folders.png)
...
### Data Safety (Checkpoint + V3 Backup)
...
**V3 Archive File Layout (v3.0.0 and later):**

Your archive now consists of two separate files:

- **`archivist.db`** — Main database. Asset metadata, tags, collections, settings.
- **`archivist_vec.db`** — Vector database. Embeddings, text chunks, file relations, keyword index.

This split reduces the main database size by 3-4× and significantly improves performance for large archives. **Both files live in the same folder** and must be **backed up together**. The built-in backup/restore flows (Snapshot, Archive Export/Import) handle the two files atomically.

> **Migration Backup:** When first migrating to V3, a copy of your old single-file archive is saved as `archivist_premigrate_v3.db.bak`. If something goes wrong, you can copy this back to `archivist.db` to restore the previous version (delete `archivist_vec.db` first). Once you have confirmed everything works, you can safely delete the `.bak` file.
...
![AI Chat Window](img/ai-chat-empty.png)
...
- **List intent detection:** Yes/no questions like "Is there an X?", "Does Y exist?" are answered directly as a file list — no waiting for the LLM, instant results.

**Example questions:**
- "Are there any renders with wooden facades?"
- "Is there a file mentioning 'Smith Foundation'?"
- "List all floor plans approved in 2024"
- "Find PDFs in projects with client name 'Smith'"
- "Which DWG files contain stairs?"

**Chat Settings (⚙ gear icon):**

![AI Chat Settings](img/ai-chat-settings.png)

The gear icon at the top-right of the chat window shows these settings:
- **Model** — Which Ollama model is in use (e.g., `qwen2.5:8b`).
- **Ollama status** — Whether the service is running.
- **Meta index** — How many files in the archive have a metadata index.
- **Content** — How many files have document text (PDF, Word, etc.) indexed.
- **Rerank** — Results re-ranked by AI (improves quality, ~10% slower).
- **Enrich** — Query expanded with synonyms (improves recall).
- **Thinking** — Show the model's reasoning steps (experimental).
...
![Multi-Archive Tabs](img/multi-archive-tabs.png)
...
![Duplicate Finder](img/duplicate-finder.png)
...
To see all changes in this version, open the **"Release Notes" tab in the Help panel** or refer to [docs/CHANGELOG.md](../../../docs/CHANGELOG.md).
