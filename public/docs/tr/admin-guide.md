...
> Sürüm 3.0.0 | 2026-05-23 — Bu kılavuz sadece Admin (tam yetkili) kullanıcılar içindir.

> **v3.0.0 Yöneticileri için Hızlı Özet:** Bu sürüm **V3 Şema Mimari**'yi getirir — vektör verisi (embedding, text chunk, ilişki, FTS) ayrı `archivist_vec.db` dosyasına taşındı. Eski arşivler ilk açılışta otomatik taşınır (`.bak` yedeği oluşur). Yedek alma akışınızı bu yeni dosya yapısına göre güncelleyin. Detaylar için aşağıdaki "4a. V3 Şema Mimari" bölümüne bakın.
...
- **Ana Arşiv (shared)** — admin yönetir, viewer salt-okunur
  - Ana DB: `archivist.db` (varsayılan: `AppDataDir/archivist.db`)
  - Vektör DB: `archivist_vec.db` (v3.0.0 sonrası — aynı klasör)
- **Yerel Arşiv (personal)** — kullanıcı bazlı
  - Ana DB: `archivist_local.db`
  - Vektör DB: `archivist_local_vec.db` (varsa)

---

## 4a. V3 Şema Mimari (3.0.0+)

![V3 Migration Paneli](img/settings-v3-migration.png)

v3.0.0 ile birlikte ArchivistPro arşiv verisini **iki ayrı SQLite dosyasında**
tutar:

| Dosya | İçerik | Tipik Boyut |
|-------|--------|-------------|
| `archivist.db` | Asset metadata, etiketler, koleksiyonlar, ayarlar, kullanıcılar, ilişki türleri | 50-100 MB |
| `archivist_vec.db` | Embedding vektörleri, metin parçaları (chunks), asset ilişkileri, FTS5 anahtar sözcük dizini | 100-300 MB |

### Neden Ayrıldı?

- **Performans:** Ana `archivist.db` 3-4× küçülür → tarayıcı SQL motorunun (sql.js)
  yüklemesi ve yazması hızlanır. Gerçek ölçüm: 181 MB → 52 MB.
- **Bellek:** Browser'da sql.js DB'yi tamamen RAM'e yükler. Daha küçük ana DB
  → daha az RAM kullanımı, daha az bloklayıcı save.
- **Ölçek:** Yüz binlerce embedding ve binlerce text-chunk'ı ayrı yapıda HNSW
  ANN indeksiyle saklamak, ana metadata DB'sini hızlı tutar.
- **Bakım:** Vektör verisi yeniden indekslenebilir; metadata kaybedilemez —
  ayrı dosyalar yedekleme stratejisini esnetir.

### Otomatik Migration

Eski sürümden geçen arşivler ilk açılışta otomatik taşınır:

1. Açılışta `PRAGMA user_version` kontrol edilir.
2. `user_version < 3` ise `runV3EpochMigration` tetiklenir.
3. **Premigrate yedek:** `archivist.db` → `archivist_premigrate_v3.db.bak`
   adıyla kopyalanır (rollback için).
4. Embedding/text_chunk/relation verisi `archivist_vec.db`'ye taşınır
   (epoch 1 → 2 → 3 sırayla).
5. Verifikasyon: kaynak ile hedef satır sayıları ve içerik birebir eşit mi?
6. Doğrulama başarılı → ana DB'deki eski tablolar `DROP TABLE` + `VACUUM` +
   `PRAGMA user_version = 3` ile temizlenir.
7. Atomik tek yazımla diske kaydedilir.

> Migration **Rust tarafında** koşar (rusqlite). 180+ MB monolitlerde Tauri
> IPC payload sınırına takılma riski yoktur.

### Manuel Tetik (Admin Panel)

Otomatik migration başarısız olur ya da kullanıcı kontrolü tercih ederse:

**Ayarlar → Depolama → V3 Şema Migrasyonu** paneli (admin-only):

- Mevcut epoch (0/1/2/3) ve hedef epoch (3) gösterilir.
- "Migrasyonu Başlat" butonu — admin'in tetikleyebileceği manuel akış.
- Adım adım ilerleme: premigrate yedek → migrate 1/3 → migrate 2/3 →
  migrate 3/3 → finalize → reload.
- Hata durumunda durur, mesaj gösterir; veri sağlam kalır
  (premigrate yedeği ile geri dönüş mümkün).

### Opt-Out

Bayrak ile V3 davranışı kapatılabilir (geliştirme/test için):

```js
localStorage.setItem('ARCHIVIST_V3_EPOCH', 'off');
```

Bayrak `off` iken otomatik migration tetiklenmez, mevcut V3 arşivleri V3
yolunda okur ama tutarsız bir state oluşmaması için **bu opt-out ancak ilk
migration ÖNCE yapılırsa anlamlıdır**. Migration sonrasında geri dönüş için
`.bak` dosyasından yedek geri yükleme gerekir.

### Rollback (Geri Dönüş)

Migration sonrası bir sorun fark ederseniz:

1. Uygulamayı kapatın.
2. `archivist_vec.db` dosyasını taşıyın veya silin.
3. `archivist_premigrate_v3.db.bak` dosyasını `archivist.db` adıyla geri
   kopyalayın (önce mevcut `archivist.db`'yi yedekleyin).
4. Uygulamayı açın — arşiv eski (epoch=0) sürümünde gelir.

### WAL Journal ve Ağ Paylaşımı (UNC)

v3.0.0 ile WAL journal mode **default açık** — yerel diskte yazma performansı
artar (~%30). Ağ paylaşımı tespit edildiğinde otomatik DELETE moduna düşülür:

- **UNC yol (`\\sunucu\paylaşım\arşiv\...`):** WAL güvensiz (NetBIOS lock
  semantikleri) → DELETE moda zorlanır.
- **Eşlenmiş ağ sürücüsü (`Z:\arşiv\...` ama gerçekte UNC):** `GetDriveTypeW`
  ile tespit edilir, DELETE moduna düşülür.
- **Yerel disk:** WAL açık kalır.

Override gerekirse env değişkeni:

```cmd
set ARCHIVIST_DB_JOURNAL=delete
```

UNC üzerinde aktif çalışıyorsanız **paylaşımı doğrudan UNC olarak açın**
(eşlenmiş sürücü harf kullanmayın) — Windows NetBIOS junction'ları edge
case yapar. Bu mod büyük çok-kullanıcılı kurulumlarda HTTP LAN paylaşımı
...
> **v3.0.0 Notu:** Snapshot **hem `archivist.db` hem `archivist_vec.db`'yi
> birlikte** kaydeder. Geri yükleme de iki dosyayı atomik olarak geri
> yükler — V3 verisi (embedding/chunk/relation) yedekten kaybedilmez.
...
- **v3.0.0:** Migration başarısız ya da disk-yazma başarısız ise akış durdurulur ve in-memory durum diskle yeniden hizalanır (sahte-başarı önlenir)
- **v3.0.0:** Per-arşiv yazma kilidi — birden çok arşiv eşzamanlı kullanılırken yazma çatışması yok
- **v3.0.0:** Stale WAL temizliği — blob-overwrite sonrası yetim `-wal`/`-shm` otomatik silinir
...
- **v3.0.0:** 5 dilde **tam içerik** mevcuttur (TR kaynak, EN/AR/JA/ZH çeviri). Yardım panelinde her sekme uygulamanın aktif diline göre yüklenir.
- Arayüz çevirileri: **5 dil %100** (TR, EN, ZH, JA, AR — 2310+ anahtar)
- AR/JA/ZH sürümleri profesyonel çeviri prosedürü ile üretilmiştir; tutarlılık için `docs/i18n-glossary.md` ortak sözlük kullanılır.
...
- **Sürüm Notları:** Yardım panelinde dördüncü sekme olarak yer alır; `docs/CHANGELOG.md` dosyasından beslenir.

---

## 16. Sürüm Bump ve Dağıtım Kontrolü (Admin)

Bu uygulamanın yeni sürümü dağıtılırken aşağıdaki kontrol listesi izlenir.
Tam metin için `docs/RELEASE_CHECKLIST.md`'ye bakın.

Özet (her sürüm bump için):

- `src/appVersion.ts` → `APP_VERSION` + `APP_BUILD_DATE` güncelle
- `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `package.json`
  sürüm alanlarını eşitle (`cargo check` Cargo.lock'ı otomatik günceller)
- `docs/CHANGELOG.md` — yeni sürüm bölümü ekle
- Her dildeki yardım dokümanları (user-guide, admin-guide, scenarios)
  tarihini güncelle
- `npx tsc --noEmit` + `npm test` + `cargo test --features admin --lib`
  yeşil olmalı
- MSI build (`npm run offline:build`) — başarılı çıktı / GitHub Release'e yükleme

---

*Bu kılavuz program geliştikçe güncellenmektedir. Son güncelleme: 2026-05-23 (v3.0.0).*
