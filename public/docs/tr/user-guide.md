...
ArchivistPro, mimari dosyalarınızı akıllı bir şekilde arşivlemenizi, aramanızı ve yönetmenizi sağlayan masaüstü uygulamasıdır. **Tamamen çevrimdışı çalışır** — dosyalarınız bilgisayarınızdan dışarı çıkmaz.

![Ana Pencere](img/main-window.png)

> **v3.0.0'a Geçiş:** Eski sürümden geliyorsanız, ilk açılışta arşiviniz otomatik olarak **V3 şemasına taşınır**. Bu işlem birkaç saniye sürer ve dokunmanıza gerek yoktur. Geriye dönüş için arşivin yanında `archivist_premigrate_v3.db.bak` yedek dosyası tutulur. Detay için aşağıdaki "Veri Güvenliği" bölümüne bakın.
...
![Kaynak Klasörler](img/sidebar-source-folders.png)
...
### Veri Güvenliği (Checkpoint + V3 Yedek)
...
**V3 Mimarinin Dosya Yapısı (v3.0.0 ve sonrası):**

Arşiviniz iki ayrı dosyadan oluşur:

- **`archivist.db`** — Ana veritabanı. Asset metadata, etiketler, koleksiyonlar, ayarlar.
- **`archivist_vec.db`** — Vektör veritabanı. Embedding'ler, metin parçaları (chunks), dosya ilişkileri, anahtar sözcük dizini.

Bu ayrım sayesinde ana veritabanı 3-4× küçülür ve büyük arşivlerde performans önemli ölçüde artar. **İkisi de aynı klasörde tutulur** ve yedek alırken **birlikte** kopyalanmalıdır. Yedekleme/geri yükleme akışları (Snapshot, Arşiv İhraç/İthal) otomatik olarak iki dosyayı birlikte ele alır.

> **Migration Yedeği:** İlk açılışta V3'e geçiş yapıldığında, eski tek dosyalı arşivinizin kopyası `archivist_premigrate_v3.db.bak` adıyla saklanır. Bir sorun olursa bu dosyayı `archivist.db` adına geri kopyalayarak eski sürüme dönebilirsiniz (`archivist_vec.db` silinmeli). Migration başarılı olduğundan emin olunca `.bak` dosyasını silebilirsiniz.
...
![AI Sohbet Penceresi](img/ai-chat-empty.png)
...
- **Liste niyeti tanıma:** "X var mı", "Y geçer mi", "Z hangi dosyada" gibi soruları doğrudan dosya listesi olarak yanıtlar — LLM beklemenize gerek yok, anında sonuç gelir.
...
**Sohbet Ayarları (⚙ çark ikonu):**

![AI Sohbet Ayarları](img/ai-chat-settings.png)

Sohbet penceresinin sağ üstündeki çark ikonu şu ayarları gösterir:
- **Model** — Hangi Ollama modelinin kullanıldığı (örn. `qwen2.5:8b`).
- **Ollama Durumu** — Servisin çalışıp çalışmadığı.
- **Meta indeks** — Arşivdeki dosyaların kaç tanesinin metadata indeksi var.
- **İçerik** — Kaç dosyanın belge metni (PDF/Word vb.) indekslendi.
- **Rerank** — Sonuçlar AI ile yeniden sıralanır (kalite artar, hız ~%10 düşer).
- **Zenginleştir** — Sorgu eş anlamlılarla genişletilir (recall artar).
- **Düşünme** — Modelin akıl yürütme adımlarını göster (deneysel).
...
![Çoklu Arşiv Sekmeleri](img/multi-archive-tabs.png)
...
![Kopya Bulucu](img/duplicate-finder.png)
...
Bu sürümde yapılan tüm değişiklikleri görmek için **Yardım panelindeki "Sürüm Notları" sekmesine** veya repo kökündeki [docs/CHANGELOG.md](../../../docs/CHANGELOG.md) dosyasına bakın.
