...
import { getChunkCountByAssetIdAsync } from '../services/database';
import {
...
export default function AssetTagsPanel({ assetId }: AssetTagsPanelProps) {
  ...
  // V3 PRE-5c: chunk sayısı epoch>=2'de vec.db'den (async) — AI öneri butonu için.
  useEffect(() => {
    let cancelled = false;
    void getChunkCountByAssetIdAsync(assetId).then((n) => {
      if (!cancelled) setChunkCount(n);
    });
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          ...
          <button
            ...
            disabled={isSuggesting || aiConfig.apiProvider !== 'ollama' || chunkCount === 0}
            title="AI ile etiket öner (indexli dosya gerekir)"
            style={{
              background: 'none', border: 'none', cursor: isSuggesting || aiConfig.apiProvider !== 'ollama' || chunkCount === 0 ? 'not-allowed' : 'pointer', padding: 2,
              color: isSuggesting || aiConfig.apiProvider !== 'ollama' || chunkCount === 0 ? 'var(--color-text-muted)' : '#a78bfa',
            }}
          >
