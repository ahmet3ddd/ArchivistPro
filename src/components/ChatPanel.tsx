...
import { askQuestionStream, askSynthesisStream, generateSessionTitle, isRerankerEnabled, setRerankerEnabled, isQueryRewriteEnabled, setQueryRewriteEnabled, getLastQueryWarnings, cleanupLlmAnswer, type RagCitation, type RagScope } from '../services/ragService';
...
export default function ChatPanel({ isOpen, onClose, aiConfig }: ChatPanelProps) {
    ...
    const refreshIndexBadge = useCallback(() => {
        try {
            // Metadata (chunk_index=-1) ve içerik (chunk_index>=0) ayrı sayılır.
            // Kullanıcı metadata vs content indexleme farkını görsün.
            const rows = queryAll(`
                SELECT
                    (SELECT COUNT(*) FROM assets WHERE is_deleted = 0) AS total,
                    (SELECT COUNT(DISTINCT asset_id) FROM text_chunks WHERE chunk_index = -1) AS meta_indexed,
                    (SELECT COUNT(DISTINCT asset_id) FROM text_chunks WHERE chunk_index >= 0) AS content_indexed
            `);
            const total = Number((rows?.[0] as unknown[] | undefined)?.[0] ?? 0);
            const metaIndexed = Number((rows?.[0] as unknown[] | undefined)?.[1] ?? 0);
            const contentIndexed = Number((rows?.[0] as unknown[] | undefined)?.[2] ?? 0);
            const missing = Math.max(0, total - metaIndexed);
            setIndexBadge({ indexed: metaIndexed, total, missing, skipped: 0, contentIndexed });
        } catch { /* ignore */ }
    }, []);
    ...
    useEffect(() => {
        ...
        const runIndexing = async (missingRows: unknown[]) => {
            try {
                ...
                for (let i = 0; i < missingRows.length; i++) {
                    ...
                    const aid = ((missingRows as unknown[])[i] as unknown[])[0] as string;
                    try {
                    ...
                    if (i % 3 === 0 || i === missingRows.length - 1) {
        ...
        const startLater = () => {
            ...
            const missingRows = needsRebuild
                ? queryAll(`SELECT id FROM assets WHERE is_deleted = 0 LIMIT 10000`)
                : queryAll(`
