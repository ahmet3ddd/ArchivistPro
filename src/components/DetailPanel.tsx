...
import { getChunkByIdAsync, getChunkCountByAssetIdAsync, getChunksByAssetIdAsync, getBackupsForAsset, updateAssetFields, getAssetById } from '../services/database';
...
export default function DetailPanel({ asset, onClose, onUpdate, matchSources, onSelectAsset }: DetailPanelProps) {
    ...
    // V3 PRE-5c: text_chunks epoch>=2'de vec.db'de → async. useMemo yerine
    // state+effect (async okuma render'da yapılamaz). epoch=0'da async kardeş
    // sync'e delege eder → 1 frame gecikme dışında davranış birebir.
    const [matchedChunk, setMatchedChunk] = useState<
        { id: string; assetId: string; chunkIndex: number; page: number | null; text: string; lang: string | null } | null
    >(null);
    useEffect(() => {
        if (!asset || !semanticResults || semanticResults.length === 0) {
            setMatchedChunk(null);
            return;
        }
        const hit = semanticResults.find((r) => r.assetId === asset.id && r.chunkId);
        if (!hit?.chunkId) { setMatchedChunk(null); return; }
        let cancelled = false;
        void getChunkByIdAsync(hit.chunkId).then((c) => {
            if (!cancelled) setMatchedChunk(c);
        });
        return () => { cancelled = true; };
    }, [semanticResults, asset]);

    const [docIndexInfo, setDocIndexInfo] = useState<
        { chunkCount: number; previews: Array<{ id: string; chunkIndex: number; page: number | null; text: string; lang: string | null }> } | null
    >(null);
    useEffect(() => {
        if (!asset) { setDocIndexInfo(null); return; }
        const DOC_TYPES = new Set(['PDF', 'DOC', 'XLS', 'PPT', 'TXT', 'CSV', 'RTF', 'SAP2K']);
        if (!DOC_TYPES.has(asset.fileType)) { setDocIndexInfo(null); return; }
        let cancelled = false;
        void (async () => {
            const chunkCount = await getChunkCountByAssetIdAsync(asset.id);
            const previewLimit = chunkPreviewExpanded ? Math.min(chunkCount, 20) : 3;
            const previews = chunkCount > 0
                ? await getChunksByAssetIdAsync(asset.id, previewLimit)
                : [];
            if (!cancelled) setDocIndexInfo({ chunkCount, previews });
        })();
        return () => { cancelled = true; };
