...
export default function SetupWizard({ onComplete }: SetupWizardProps) {
  ...
  // fp32 modelleri opsiyonel — WebGPU hızlandırma. Bulunamazsa akış BLOKLANMAZ.
  const [fp32Status, setFp32Status] = useState<'checking' | 'found' | 'notfound'>('checking');
  ...
  // fp32 modelleri kontrolü (opsiyonel — WebGPU hızlandırma)
  const checkFp32 = useCallback(async () => {
    setFp32Status('checking');
    try {
      const { invoke } = await import('@tauri-apps/api/core');
      const s = await invoke<{ imported: boolean }>('fp32_models_status');
      setFp32Status(s.imported ? 'found' : 'notfound');
    } catch {
      setFp32Status('notfound');
    }
  }, []);
...
function StepChecklist({ ollama, odaStatus, fp32Status, onRecheckOllama, onRecheckOda, onRecheckFp32 }: {
  ollama: OllamaStatus;
  odaStatus: 'checking' | 'found' | 'notfound';
  fp32Status: 'checking' | 'found' | 'notfound';
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {/* fp32 modelleri (opsiyonel — WebGPU hızlandırma; akışı bloklamaz) */}
        <CheckRowWithAction
          ok={fp32Status === 'found'}
          checking={fp32Status === 'checking'}
          label={t('wizard.checklist.fp32Models')}
          detail={fp32Status === 'checking'
            ? t('wizard.checklist.checking')
            : fp32Status === 'found'
              ? t('wizard.checklist.fp32Found')
              : t('wizard.checklist.fp32NotFound')
          }
          hint={fp32Status === 'notfound' ? t('wizard.checklist.fp32Hint') : undefined}
          onRecheck={onRecheckFp32}
        />
      </div>
