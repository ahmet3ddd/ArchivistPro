...
import { getAllEmbeddingsAsync, getEmbeddingsBySourcePrefixAsync, getAssetPhashMap } from '../services/database';
    ...
    async (file: File) => {
      ...
      if (aiConfig.enableClipVision) {
        try {
          ...
          const sourceVectors = await getEmbeddingsBySourcePrefixAsync('image_');
          const legacyVectors = sourceVectors.length === 0 ? await getAllEmbeddingsAsync('image') : [];
