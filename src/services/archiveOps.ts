    ...
    await withArchive(opts.targetId, async () => {
        ...
        if (opts.includeEmbeddings) {
            opts.onProgress?.({
                ...
                current: 0,
                total: 0,
                message: 'AI vektörleri kopyalanıyor',
            });
            try {
                const embeddings = getAllEmbeddingsFromArchive(opts.sourceId);
                let idx = 0;
                for (const emb of embeddings) {
                    idx++;
                    if (skippedIds.has(emb.assetId)) continue;
                    const targetAssetId = idMap.get(emb.assetId) ?? emb.assetId;
                    try {
                        if (emb.refId) {
                            // Chunk embedding — refId orijinal chunk id, yeni chunk id gerekebilir (Phase 4'te)
                            saveChunkEmbedding(targetAssetId, emb.refId, emb.vector, emb.source);
                        } else {
                            saveEmbedding(targetAssetId, emb.vector, emb.source);
                        }
                        result.embeddingsCopied++;
                    } catch (err) {
                        result.errors.push(`Embedding ${emb.id}: ${String(err)}`);
                    }
                    if (idx % 50 === 0) {
                        opts.onProgress?.({
                            phase: 'embeddings',
                ...
                result.errors.push(`Embeddings: ${String(err)}`);
            }
        }

        // ── Phase 4: Text Chunks ──
        if (opts.includeTextChunks) {
            opts.onProgress?.({
                phase: 'chunks',
                current: 0,
                total: 0,
                message: 'Metin parçaları kopyalanıyor',
            });
            try {
                const chunks = getAllTextChunksFromArchive(opts.sourceId);
                let idx = 0;
                for (const chunk of chunks) {
                    idx++;
                    if (skippedIds.has(chunk.assetId)) continue;
                    const targetAssetId = idMap.get(chunk.assetId) ?? chunk.assetId;
                    try {
                        upsertTextChunk({
                            id: chunk.id,
                        ...
                        result.errors.push(`Chunk ${chunk.id}: ${String(err)}`);
                    }
                    if (idx % 100 === 0) {
                        opts.onProgress?.({
                            phase: 'chunks',
                ...
                result.errors.push(`TextChunks: ${String(err)}`);
            }
        }

        // ── Phase 5: Asset Summaries ──
...
export async function extractAssets(opts: ExtractOptions): Promise<ExtractResult> {
    ...
    try {
        await withArchive(targetId, async () => {
            ...
            // Phase 3: Embeddings
            if (opts.includeEmbeddings) {
                opts.onProgress?.({ phase: 'embeddings', current: 0, total: 0, message: 'AI vektörleri kopyalanıyor' });
                try {
                    const embeddings = getAllEmbeddingsFromArchive(opts.sourceId);
                    let idx = 0;
                    for (const emb of embeddings) {
                        idx++;
                        if (!matchedIds.has(emb.assetId)) continue;
                        try {
                            if (emb.refId) {
                                saveChunkEmbedding(emb.assetId, emb.refId, emb.vector, emb.source);
                            } else {
                                saveEmbedding(emb.assetId, emb.vector, emb.source);
                            }
                            result.embeddingsCopied++;
                        } catch (err) {
                            result.errors.push(`Embedding ${emb.id}: ${String(err)}`);
                        ...
                        if (idx % 50 === 0) {
                            opts.onProgress?.({ phase: 'embeddings', current: idx, total: embeddings.length, message: 'AI vektörleri kopyalanıyor' });
                            await yieldToUi();
                        }
                    }
                } catch (err) {
            ...
            // Phase 4: Text Chunks
            if (opts.includeTextChunks) {
                opts.onProgress?.({ phase: 'chunks', current: 0, total: 0, message: 'Metin parçaları kopyalanıyor' });
                try {
                    const chunks = getAllTextChunksFromArchive(opts.sourceId);
                    let idx = 0;
                    for (const chunk of chunks) {
                        idx++;
                        if (!matchedIds.has(chunk.assetId)) continue;
                        try {
                            upsertTextChunk({
                                id: chunk.id,
                        ...
                        if (idx % 100 === 0) {
                            opts.onProgress?.({ phase: 'chunks', current: idx, total: chunks.length, message: 'Metin parçaları kopyalanıyor' });
                            await yieldToUi();
                        }
                    }
                } catch (err) {
