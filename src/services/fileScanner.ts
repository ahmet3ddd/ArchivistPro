...
import { upsertAsset, saveEmbedding, saveDatabase, getSetting, getAssetById, upsertTextChunk, saveChunkEmbedding, getChunkCountByAssetId, getChunksByAssetId, deleteTextChunksByAssetId, deleteChunkEmbeddingsByAssetId, detectAndSaveSameStemRelations, getActiveArchive } from './database';
    ...
    async flush(): Promise<boolean> {
        ...
        try {
            ...
            console.info(
                ...
                `shapes=${result.shapes_written}/${sizes.shapes} ` +
                `shapeDeletes=${result.shapes_deleted} ` +
...
export async function scanDirectory(
    ...
    async function ensureDocumentChunksIndexed(asset: Asset, filePath: string): Promise<void> {
        ...
        const existing = getChunkCountByAssetId(asset.id);
        if (existing > 0) {
            const preview = getChunksByAssetId(asset.id, 1)[0]?.text || '';
    ...
    async function processEntry(prep: PrepResult, entry: typeof fileEntries[0]): Promise<void> {
        ...
        try {
            ...
            // Faz 4.1 + 4.2 — Shape persist (FK: asset_id → assets.id, upsertAsset SONRASI olmak zorunda)
            // Dual-write: sql.js (UI shape arama runtime) + rusqlite (kalıcılık, final saveDatabase'i öldürür).
            if (prep.shapeResult) {
                try {
                    const { persistDwgShapes, categorizeLayerForShape } = await import('./dwgShapeIndex');
                    if (prep.shapeResult.shapes.length > 0) {
                        persistDwgShapes(asset.id, prep.shapeResult.shapes);
                        writeBuffer.markDeleteShapes(asset.id);
    ...
    // Aynı-stem ilişkileri otomatik tespit — writeBuffer'a mirror et (sql.js INSERT'leri zaten yapılır).
    // skipSave: true → fonksiyon içindeki saveDatabase atlanır; persist writeBuffer.flush üzerinden olur.
    try {
        const relCount = detectAndSaveSameStemRelations(
            assets,
        ...
        debugLog('Scanner', 'detectAndSaveSameStemRelations error', err);
    }
