 ...
 *
 * Disk yazımı: saveDatabase() yerine tablo-özel tag_apply_changes invoke
 * (asset_tags + root_tags da aynı komutta birlikte yönetilir).
 */

import { auditLog, debugLog } from './logger';
import { getActiveArchive, getChunksByAssetIdAsync } from './database';
...
/* ── Mirror helper (sql.js → rusqlite, archive-aware) ──
 * tags + asset_tags + root_tags birlikte yönetilir.
 * Backend tag_deletes geldiğinde asset_tags/root_tags'i cascade siler (FK OFF).
 */

export interface TagMirrorPayload {
  tag_inserts?: Array<{ id: number; name: string; color?: string; created_at?: string }>;
  tag_renames?: Array<{ id: number; name: string }>;
  tag_color_updates?: Array<{ id: number; color: string }>;
  tag_deletes?: number[];
  asset_tag_inserts?: Array<{ asset_id: string; tag_id: number }>;
  asset_tag_deletes?: Array<{ asset_id: string; tag_id: number }>;
  asset_tag_clear_for_assets?: string[];
  asset_tag_clear_for_tags?: number[];
  asset_tag_merges?: Array<{ source_tag_id: number; target_tag_id: number }>;
  root_tag_inserts?: Array<{ root_id: string; tag_id: number }>;
  root_tag_deletes?: Array<{ root_id: string; tag_id: number }>;
  root_tag_clear_for_roots?: string[];
}

export async function mirrorTagChangesToDisk(payload: TagMirrorPayload): Promise<void> {
  const hasWork = (payload.tag_inserts?.length ?? 0) > 0
    || (payload.tag_renames?.length ?? 0) > 0
    || (payload.tag_color_updates?.length ?? 0) > 0
    || (payload.tag_deletes?.length ?? 0) > 0
    || (payload.asset_tag_inserts?.length ?? 0) > 0
    || (payload.asset_tag_deletes?.length ?? 0) > 0
    || (payload.asset_tag_clear_for_assets?.length ?? 0) > 0
    || (payload.asset_tag_clear_for_tags?.length ?? 0) > 0
    || (payload.asset_tag_merges?.length ?? 0) > 0
    || (payload.root_tag_inserts?.length ?? 0) > 0
    || (payload.root_tag_deletes?.length ?? 0) > 0
    || (payload.root_tag_clear_for_roots?.length ?? 0) > 0;
  if (!hasWork) return;
  try {
    const { invoke } = await import('@tauri-apps/api/core');
    await invoke('tag_apply_changes', { payload, archiveAt: getActiveArchive() });
  } catch (err) {
    debugLog('TagService', 'mirrorTagChangesToDisk error', err);
  }
}
...
export function createTag(name: string, color = '#6366f1'): Tag | null {
  ...
  try {
    ...
    void mirrorTagChangesToDisk({
      tag_inserts: [{ id: tag.id, name: tag.name, color: tag.color, created_at: tag.createdAt }],
    });
    return tag;
  } catch (err) {
...
export function renameTag(tagId: number, newName: string): boolean {
  ...
  try {
    ...
    void mirrorTagChangesToDisk({
      tag_renames: [{ id: tagId, name: newName.trim() }],
    });
    return true;
  } catch {
    return false;
...
export function updateTagColor(tagId: number, color: string): boolean {
  ...
  try {
    ...
    void mirrorTagChangesToDisk({
      tag_color_updates: [{ id: tagId, color }],
    });
    return true;
  } catch {
    return false;
...
export function restoreTag(snap: { tag: Tag; assetIds: string[] }): boolean {
  ...
  try {
    ...
    void mirrorTagChangesToDisk({
      tag_inserts: [{ id: snap.tag.id, name: snap.tag.name, color: snap.tag.color, created_at: snap.tag.createdAt }],
      asset_tag_inserts: snap.assetIds.map(aid => ({ asset_id: aid, tag_id: snap.tag.id })),
    });
    return true;
  } catch {
    return false;
...
export function deleteTag(tagId: number): boolean {
  ...
  try {
    ...
    // Backend tag_deletes geldiğinde asset_tags + root_tags da otomatik temizler
    void mirrorTagChangesToDisk({ tag_deletes: [tagId] });
    return true;
  } catch {
    return false;
  }
...
export function mergeTags(sourceTagId: number, targetTagId: number): boolean {
  ...
  try {
    ...
    _db.run('COMMIT');
    // Backend: merge (INSERT-SELECT) + tag_deletes (asset_tags/root_tags cascade)
    void mirrorTagChangesToDisk({
      asset_tag_merges: [{ source_tag_id: sourceTagId, target_tag_id: targetTagId }],
      tag_deletes: [sourceTagId],
    });
    return true;
  } catch (err) {
...
export function addTagToAsset(assetId: string, tagId: number): boolean {
  ...
  try {
    ...
    void mirrorTagChangesToDisk({
      asset_tag_inserts: [{ asset_id: assetId, tag_id: tagId }],
    });
...
export function removeTagFromAsset(assetId: string, tagId: number): boolean {
  ...
  try {
    ...
    void mirrorTagChangesToDisk({
      asset_tag_deletes: [{ asset_id: assetId, tag_id: tagId }],
    });
...
export function setTagsForAsset(assetId: string, tagIds: number[]): boolean {
  ...
  try {
    ...
    _db.run('COMMIT');
    void mirrorTagChangesToDisk({
      asset_tag_clear_for_assets: [assetId],
      asset_tag_inserts: tagIds.map(tid => ({ asset_id: assetId, tag_id: tid })),
    });
    return true;
  } catch (err) {
    try { _db.run('ROLLBACK'); } catch { /* ignore */ }
...
export async function suggestTagsForAsset(assetId: string, aiConfig: AIConfig): Promise<string[]> {
  ...
  const chunks = await getChunksByAssetIdAsync(assetId, 8);
