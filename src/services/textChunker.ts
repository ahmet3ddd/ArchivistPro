...
export async function indexAssetMetadata(
    ...
    // V3 PRE-6c: epoch>=2'de text_chunks/embeddings vec.db'de — sql.js DELETE
    // "no such table" atardı; eski meta chunk vec.db'den temizlenir
    // (`vec_db_delete_metadata_chunks`, body chunk'lara dokunmaz).
    const epoch = getSchemaEpoch();
    if (epoch >= 2) {
        await deleteMetadataChunksFromVecDb(assetId);
    } else {
        // epoch 0/1: text_chunks sql.js'te. epoch=1'de embeddings vec.db'de →
        // o DELETE atlanır (yeni meta emb'i mirrorRagWriteToDisk vec.db'ye yazar).
        // FTS5 virtual table sql.js WASM'de `LIKE` desteklemiyor → önce
        // text_chunks'tan eski meta chunk id'leri çek, sonra tam eşleşme ile sil.
        if (epoch < 1) {
