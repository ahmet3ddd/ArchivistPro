...
describe('snapshotScannedRootWithAssets & restoreScannedRootWithAssets', () => {
  ...
  it('snapshot captures root and assets, restore brings them back', () => {
    const rootId = addScannedRoot('C:/Projects');
    upsertAsset(makeAsset({ id: 'a1', filePath: 'C:/Projects/file1.dwg' }));

    const snap = snapshotScannedRootWithAssets(rootId);
    ...
    // Restore
    restoreScannedRootWithAssets(snap!);
    expect(getScannedRoots()).toHaveLength(1);
    expect(getAssetById('a1')).not.toBeNull();
  });

  it('snapshot returns null for non-existing root', () => {
    expect(snapshotScannedRootWithAssets('nonexistent')).toBeNull();
  });

  it('snapshot with no assets under root returns empty arrays', () => {
  ...
  it('snapshot returns null when db is null', () => {
    _setDbForTesting(null);
    expect(snapshotScannedRootWithAssets('x')).toBeNull();
  });
});
